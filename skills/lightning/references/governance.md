...
> The governance hub of this skill. Start here when building private voting for a DAO, a token holder vote, a council election, a grants round, or any "collect sealed choices, publish a result" flow. It decides WHAT stays private and WHEN it reveals, then maps that onto Inco primitives. The worked reference is [scripts/governance/ConfidentialBallot.sol](../scripts/governance/ConfidentialBallot.sol) with its Foundry tests and [examples/confidential-vote.ts](../examples/confidential-vote.ts). The base API is on the [main skill page](../SKILL.md).
...
- [ ] Tie rule documented (reference: lowest option index wins in `WinnerOnly` mode)
- [ ] Foundry tests run green with `IncoTest` (the reference ships nine, including swapped-attestation rejection)
